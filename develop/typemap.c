...
cross_make_int_token(long long value) {
	 ...
	 */
	ret->data = zalloc_buf(Z_CEXPR_BUF); /*n_xmalloc(16);*/ /* XXXXXXX */
...
cross_get_pow2_minus_1(struct token *tok) {
	...
	if (tok->type == TY_UINT
		...
		if ((uval & (uval - 1)) == 0) {
			...
			res = alloc_token();
			res->type = tok->type;
			res->data = zalloc_buf(Z_CEXPR_BUF);  /*n_xmalloc(16);*/ /* XXX */
...
cross_convert_const_token(struct token *tok, int newty) {
	...
	tv.type = make_basic_type(tok->type);
	tv.value = zalloc_buf(Z_CEXPR_BUF);  /*n_xmalloc(16);*/
...
cross_exec_op_for_const_tokens(struct token **result,
	...
	restv.value = zalloc_buf(Z_CEXPR_BUF); /*n_xmalloc(16);*/ /* XXXXXXXXXXX */
...
cross_exec_op(struct token *optok,
	...
	if (type_map[idx] != NULL) {
		...
		res->alloc = 1;
		res->value = zalloc_buf(Z_CEXPR_BUF);  /*n_xmalloc(16);*/ /* XXX */
...
#else
		if (base_bytes > cross_get_sizeof_type(make_basic_type(TY_UINT))) {
#endif
...
#else
	if (base_bytes > cross_get_sizeof_type(make_basic_type(TY_ULONG))) {
#endif
		/*
	...
	if (d->dtype->sign != TOK_KEY_UNSIGNED) {
		...
		if (1) { /*target_info->arch_info->endianness == ENDIAN_LITTLE) {*/
			...
			if (backend->arch == ARCH_MIPS) {
				/*
				 * 08/01/09: If the storage unit is a 32bit
				 * int, then the calculated number of bits
				 * doesn't sign-extend! Because for a 7bit
				 * item, the number is 25, but
				 *    sll val, 25;   sra val, 25
				 * should really be
				 *    dsll val, 57;  dsra val, 57
				 * ... to sign-extend to a 64bit GPR
				 * XXX Does this apply to other systems?
				 * This problem didn't appear to happen on
				 * PPC64 and SPARC64
				 */
		...
		if (extbits == 0) {
			...
			if (need_llong_token) {
				d->dtype->tbit->shifttok_signext_left->flags |= TOK_FLAG_LONG_SHIFT;
			}
		}
		if (revbits == 0) {
			...
			if (need_llong_token) {
				d->dtype->tbit->shifttok_signext_right->flags |= TOK_FLAG_LONG_SHIFT;
			}
		}
	} else {
		d->dtype->tbit->shifttok_signext_left = NULL;
